...
class State():
    ...
    #    3: on
    #   -3: under
	def __init__(self, graph=None, moves=[]):
        if graph is None:
            graph = np.zeros([self.num_objects, self.num_objects])
		self.graph=graph
		self.moves=moves
	...
	def next_state(self):
        nextmove = random.choice(self.possible_moves)
        new_graph = copy.deepcopy(self.graph)
        new_graph[nextmove[0], nextmove[1]] = nextmove[2]
        new_graph[nextmove[1], nextmove[0]] = -nextmove[2]
		next = State(new_graph, self.moves+[nextmove])
		return next
