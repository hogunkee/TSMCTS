...
sys.path.append(os.path.join(FILE_PATH, '../..', 'BLIP'))
...
class MCTS(object):
    ...
    def setBLIPModel(self, blipModel, preprocess, text):
        self.blip = blipModel
        self.blipPreProcess = preprocess
        self.text = text #clip.tokenize([text]).cuda()
    ...
    def getReward(self, tables, groundTruth=False):
        ...
        for table in tables:
            ...
            state = self.blipPreProcess(Image.fromarray(rgb)).unsqueeze(0)
            states.append(state)
        ...
        if len(states) > self.batchSize:
            ...
            for b in range(numBatches):
                ...
                if self.similarity_score:
                    # Calculate features
                    with torch.no_grad():
                        batchRewards = self.blip(batchStates, self.text, match_head='itc')
                else:
                    with torch.no_grad():
                        itm_output = self.blip(batchStates, self.text, match_head='itm')
                        batchRewards = torch.nn.functional.softmax(itm_output, dim=1)[:,1]
            ...
            if self.similarity_score:
                # Calculate features
                with torch.no_grad():
                    rewards = self.blip(s_image, self.text, match_head='itc')
            else:
                with torch.no_grad():
                    itm_output = self.blip(s_image, self.text, match_head='itm')
                    rewards = torch.nn.functional.softmax(itm_output, dim=1)[:,1]
...
if __name__=='__main__':
    ...
    # BLIP
    parser.add_argument('--blip-model', type=str, default="ViT-L/14@336px")
