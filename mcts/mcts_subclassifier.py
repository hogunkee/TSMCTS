...
class MCTS(object):
    ...
    def setSubRewardNet(self, subRewardNet):
        self.subRewardNet = subRewardNet
        ...
        if len(states) > self.batchSize:
            ...
            for b in range(numBatches):
                ...
                if groundTruth or self.rewardType=='gt':
                    gtRewards = self.gtRewardNet(batchStatesR).cpu().detach().numpy()
                    subRewards = self.subRewardNet(batchStatesR).cpu().detach().numpy()
                    batchRewards = (1-self.alpha) * gtRewards + self.alpha * subRewards
                else:
...
if __name__=='__main__':
    ...
    parser.add_argument('--iteration-limit', type=int, default=20000)
    ...
    parser.add_argument('--num-rotate', type=int, default=2)
    # Reward model
    ...
    parser.add_argument('--sub-reward-model-path', type=str, default='data/classification/0624_2150/line_nobg_binary_mse-best.pth')
    ...
    sub_model_path = args.sub_reward_model_path
    subRewardNet, _ = loadRewardFunction(sub_model_path)
    searcher.setSubRewardNet(subRewardNet)
