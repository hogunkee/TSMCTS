...
if __name__=='__main__':
    ...
    for sidx in bar:
        ...
        if args.logging:
            Image.fromarray(initRgb).save('%s-%s/scene-%d/top_initial.png'%(log_dir, log_name, sidx))
            Image.fromarray(initRgbFront).save('%s-%s/scene-%d/front_initial.png'%(log_dir, log_name, sidx))
            Image.fromarray(initRgbNV).save('%s-%s/scene-%d/nv_top_initial.png'%(log_dir, log_name, sidx))
            Image.fromarray(initRgbFrontNV).save('%s-%s/scene-%d/nv_front_initial.png'%(log_dir, log_name, sidx))
            Image.fromarray(initSeg).save('%s-%s/scene-%d/top_seg_init.png'%(log_dir, log_name, sidx))
            Image.fromarray(initSegNV).save('%s-%s/scene-%d/top_seg_init_nv.png'%(log_dir, log_name, sidx))
        ...
        for step in range(10):
            ...
            if args.logging and actionProb is not None:
                ...
                Image.fromarray(actionProb).save('%s-%s/scene-%d/actionprob_%d.png'%(log_dir, log_name, sidx, step))
            ...
            if args.logging:
                ...
                Image.fromarray(tableRgb).save('%s-%s/scene-%d/expect_%d.png'%(log_dir, log_name, sidx, step))
            ...
            if args.logging:
                Image.fromarray(currentRgb).save('%s-%s/scene-%d/top_real_%d.png'%(log_dir, log_name, sidx, step))
                Image.fromarray(currentRgbFront).save('%s-%s/scene-%d/front_real_%d.png'%(log_dir, log_name, sidx, step))
                Image.fromarray(currentRgbNV).save('%s-%s/scene-%d/nv_top_real_%d.png'%(log_dir, log_name, sidx, step))
                Image.fromarray(currentRgbFrontNV).save('%s-%s/scene-%d/nv_front_real_%d.png'%(log_dir, log_name, sidx, step))
                Image.fromarray(currentSeg).save('%s-%s/scene-%d/top_seg_%d.png'%(log_dir, log_name, sidx, step))
                Image.fromarray(currentSegNV).save('%s-%s/scene-%d/top_seg_%d_nv.png'%(log_dir, log_name, sidx, step))
            ...
            if terminal:
                ...
                print_fn("--------------------------------")
                if args.logging:
                    Image.fromarray(currentRGB).save('%s-%s/scene-%d/top_final.png'%(log_dir, log_name, sidx))
                    Image.fromarray(currentRgbFront).save('%s-%s/scene-%d/front_final.png'%(log_dir, log_name, sidx))
                    Image.fromarray(currentRgbNV).save('%s-%s/scene-%d/nv_top_final.png'%(log_dir, log_name, sidx))
                    Image.fromarray(currentRgbFrontNV).save('%s-%s/scene-%d/nv_front_final.png'%(log_dir, log_name, sidx))
                    Image.fromarray(currentSeg).save('%s-%s/scene-%d/top_seg_final.png'%(log_dir, log_name, sidx))
                    Image.fromarray(currentSegNV).save('%s-%s/scene-%d/top_seg_final_nv.png'%(log_dir, log_name, sidx))
        ...
        if args.logging and bestRgb is not None:
            Image.fromarray(bestRgb).save('%s-%s/scene-%d/top_best.png'%(log_dir, log_name, sidx))
            Image.fromarray(bestRgbFront).save('%s-%s/scene-%d/front_best.png'%(log_dir, log_name, sidx))
