...
class TreeNode():
    def __init__(self, state, parent):
        ...
        self.isFullyExpanded = False #self.isTerminal
        self.parent = parent
        self.numVisits = 0
        self.totalReward = 0
        self.children = {}
...
class MCTS():
    def __init__(self, object_info, classifier=None, timeLimit=None, iterationLimit=None, explorationConstant=1/math.sqrt(2)):
        ...
        self.rollout = self.classifierPolicy #self.randomPolicy
    ...
    def classifierPolicy(self, node):
        state = node.state
        depth = node.depth
        table_rgb = self.object_info.grid2rgb(state.table)
        score = self.classifier(torch.Tensor(table_rgb[np.newaxis, :]).permute([0,3,1,2]).cuda())
        score = float(score[0][0])
        score = 2*(score-0.2)
        scores = [score]
        isTerminal = score>0.9
        #while not state.isTerminal() and depth<MAX_DEPTH:
        while not isTerminal and depth<MAX_DEPTH:
            try:
                action = random.choice(state.getPossibleActions())
            except IndexError:
                raise Exception("Non-terminal state has no possible actions: " + str(state))
...
if __name__=='__main__':
    classifier_model_path = 'data/classification/nobg_best.pth'
    classifier = resnet18(pretrained=False)
    fc_in_features = classifier.fc.in_features
    ...
    if resize:
        init_rgb = np.array(Image.open(rgb_list[0]).resize((200, 200)))
        init_seg = Image.fromarray(np.flip(np.load(seg_list[0]), 0)).resize((200, 200))
        init_seg = np.round(init_seg).astype(int)
    ...
    #searcher = MCTS(obj_info, classifier, timeLimit=2000)
    searcher = MCTS(obj_info, classifier, iterationLimit=3000)
    print("--------------------------------")    
    print("Initial State:")
    print(state)
    for s in range(10):
        ...
        score = classifier(torch.Tensor(table_rgb[np.newaxis, :]).permute([0,3,1,2]).cuda())
        score = float(score[0][0])
        score = 2*(score-0.2)
