...
class DDPM_Vision_Condition(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # return MSE between added noise, and our predicted noise
        return self.loss_mse(noise, self.nn_model(x_t, _ts / self.n_T, xf_proj=c, context_mask=context_mask))
        #return self.loss_mse(noise, self.nn_model(x_t, c, _ts / self.n_T, context_mask))
    ...
    def sample(self, n_sample, size, device, context, guide_w = 0.0):
        ...
        for i in range(self.n_T, 0, -1):
            ...
            # split predictions and compute weighting
            eps = self.nn_model(x_i, t_is, xf_proj=c_i, context_mask=context_mask)
            #eps = self.nn_model(x_i, c_i, t_is, context_mask)
            eps1 = eps[:n_sample]
            eps2 = eps[n_sample:]
            eps = (1+guide_w)*eps1 - guide_w*eps2
