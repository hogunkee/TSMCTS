...
for ns in range (int(opt.nb_scenes)):
    # set objects #
    for idx, obj in enumerate(ids_pybullet_and_nvisii_names):
        pos_hidden = [xx[idx], yy[idx], 0.5]
    ...
    for nf in range(int(opt.nb_frames)):
        ...
        for j in range(2000):
            p.stepSimulation()
            vel_linear, vel_rot = get_velocity(ids_pybullet_and_nvisii_names)
            ...
            if j%10==0:
                if stop_linear and stop_rotation:
                    break
        sync_object_poses(ids_pybullet_and_nvisii_names)
